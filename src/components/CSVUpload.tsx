  ...
  const handleFileUpload = async (
    ...
    reader.onload = async (e) => {
      try {
        ...
        const parsedData: EnvironmentalData[] = lines
          .slice(1)
          .map((line) => {
            const values = line
              .split(";")
        ...
        const resposta = await fetch("/api/leituras/lote", {
          method: "POST",
          headers: {
        ...
        if (!resposta.ok) {
          throw new Error("Erro ao salvar CSV no backend");
        }

        onDataLoaded(parsedData);

        alert(
          `${parsedData.length} leituras salvas com sucesso no banco.`
        );
      } catch (error) {
        console.error(error);
